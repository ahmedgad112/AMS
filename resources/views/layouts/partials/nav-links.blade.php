...
<a href="{{ route('dashboard') }}"
   @if($mobile) @click="mobileNavOpen = false" @endif
...
<a href="{{ route('activity-log.index') }}"
   @if($mobile) @click="mobileNavOpen = false" @endif
   class="flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm font-medium transition {{ request()->routeIs('activity-log.*') ? 'bg-indigo-600 text-white' : 'text-slate-300 hover:bg-slate-800 hover:text-white' }}">
